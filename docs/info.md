#  react context api for state management & exif library for client side exif parsing

pass the uploaded image from the `/upload` page to the `/decode` page and then process it on the `/decode` page to extract and display its `exif` data

1.  project dependencies

add the `exifr` library to handle the `exif` data extraction -> `npm install exifr`

2.  **[src/app/AppContext.tsx](#srcappappcontexttsx)**  create a shared application context

to share the uploaded file between the `/upload` and `/decode` pages, create a new context  
new file at this path `src/app/AppContext.tsx` with the following content.  
this context will manage the state of the uploaded file and it's preview url, and the extracted exif data

3.  **[src/app/layout.tsx](#srcapplayouttsx)**  update the root layout 

wrap the entire application with a new `AppProvider` so that all pages have access to the shared state and modify the existing layout to include the `AppProvider`

4.  **[src/app/upload/page.tsx](#srcappuploadpagetsx)**  update the upload page 
...
export type FullExifData = Record<string, any>;
...
export default function UploadPage() {
    ...
    const { uploadedFile, setUploadedFile, imagePreviewUrl, setImagePreviewUrl } = appContext;
    ...
    const handleDragOver = (event: DragEvent<HTMLDivElement>) => {
        ...
        if (!fileInfo) setIsDraggingOver(true);
    };
    ...
    return (
        <div className="app-container">
            ...
            <main className="app-main upload-page-main">
                {!fileInfo ? (
                    <div className={`upload-dropzone ${isDraggingOver ? 'drag-over' : ''}`} onClick={handleUploadAreaClick} onDragOver={handleDragOver} onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDrop={handleDrop} role="button" tabIndex={0} aria-label="Upload image dropzone">
                        <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".png,.jpeg,.jpg,.heic,.jp2,.tiff,.tif" style={{ display: 'none' }} />
